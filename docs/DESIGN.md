# Design System - Sistem Pengajuan Gudang
...
## 0. Internal Tools Design Principles

### SaaS vs Internal Tools
This is an **internal enterprise application**, not a SaaS product. Design decisions prioritize:

| SaaS Product | Internal Tool |
|--------------|---------------|
| Marketing polish | Operational efficiency |
| Customer acquisition | Task completion speed |
| Feature discovery | Power user workflows |
| Onboarding flows | Muscle memory & shortcuts |
| Visual wow factor | Information density & clarity |

### Design Priorities
1. **Information Density**: Staff use this 8+ hours/day. Pack more data per screen without clutter.
2. **Action Clarity**: Primary actions (Approve, Reject, Submit) must be instantly identifiable.
3. **Status Visibility**: Approval state should be obvious at a glance from any screen.
4. **Keyboard Efficiency**: Minimize mouse dependency for power users.
5. **Reduced Eye Strain**: Warm cream canvas (`#F3F0EE`) for all-day comfort.

### Stitch MCP Configuration
- **Model**: Always use `GEMINI_3_1_PRO` for screen generation
- **Device**: Desktop-first (2560px width), responsive down to mobile
- **Theme**: Warm cream editorial, pill shapes, Inter typography
...
## 10. Sistem Pengajuan Gudang - UI Components

### Application Context

Design system ini diterapkan pada **Sistem Pengajuan Gudang** — aplikasi pengajuan pembangunan gudang distribusi dengan approval berjenjang (6 levels).

### Semantic Colors (Application-Specific)

| Status | Background | Text | Usage |
|--------|------------|------|-------|
| Approved | `#DCFCE7` | `#166534` | Status badge, success states |
| Pending | `#FEF3C7` | `#92400E` | Status badge, waiting states |
| Rejected | `#FEE2E2` | `#991B1B` | Status badge, error states |
| Input Focus | — | `#CF4500` | Border highlight |
| Error | `#DC2626` | `#DC2626` | Validation messages |
...
**Layout:**
- Full viewport height, Canvas Cream background
- Centered login card (max-width 420px)
- Logo/company name di bagian atas card
- Form fields: Username, Password
- Primary button: "Masuk" (Ink Black pill)
- Link "Lupa Password?" di bawah form

**Visual Elements:**
- Card: White, 32px radius, shadow level 2
- Inputs: 16px radius, subtle border, focus = Signal Orange
- Button: 20px radius, full width
- Internal card padding: 48px

**User Flow:**
1. Enter credentials → click "Masuk"
2. Redirect ke 2FA verification
3. Enter TOTP code dari Google Authenticator
4. Redirect ke Dashboard

---

### 11.2 2FA Verification Page

**Layout:**
- Centered card (similar to login)
- Title: "Verifikasi Dua Faktor"
- Subtitle: "Masukkan kode dari Google Authenticator Anda"
- 6-digit input (single field atau 6 separate boxes)
- Button: "Verifikasi"

**Visual Elements:**
- Icon: Shield/lock illustration di atas title
- Input: Large, centered, monospace font untuk digits
- Auto-submit setelah 6 digits entered
...
**Top Navigation:**
- Floating white pill (999px radius)
- Logo kiri, user name + role kanan, logout button

**Main Content:**
- Page title: "Dashboard Pengajuan"
- Stats cards row (3 cards): Total Pengajuan, Menunggu Approval, Disetujui Bulan Ini
- Submissions table dengan columns: ID, Nama Gudang, Lokasi, Budget, Status, Step, Aksi
- Filter dropdown: All, Pending, Approved, Rejected
- Search bar
- "Buat Pengajuan Baru" button (primary)

**Table Row Design:**
- Pill-shaped rows (24px radius)
- Hover: slightly darker cream
- Status badges dengan color semantic

---

### 11.4 New Submission Form
...
**Sections:**
1. **Informasi Gudang**: Nama, Alamat, Provinsi, Kota/Kabupaten
2. **Koordinat Lokasi**: Lat/Lng inputs, embedded map preview, "Ambil dari Peta" button
3. **Estimasi Budget**: Currency input (Rp format), optional breakdown
4. **Dokumen Pendukung**: Drag-drop upload, minimum 3 dokumen, progress indicator
5. **Keterangan Tambahan**: Notes textarea (optional)

**Action Buttons:**
- "Batal" (secondary, outlined)
- "Kirim Pengajuan" (primary, Ink Black)
- Sticky bottom bar pada mobile

---

### 11.5 Submission Detail Page
...
**Header Card:**
- ID Pengajuan: #WH-2026-0001
- Status badge besar
- Tanggal Pengajuan

**Warehouse Info Card:** Nama, Alamat, Koordinat icon

**Budget Card:** Estimated Budget (large typography), breakdown table

**Documents Card:** List dengan file icon, nama, tanggal, download/preview buttons

**Map Card:** Interactive map (Leaflet), marker, zoom controls, "Buka di Google Maps" link

**Right Column (30%):**

**Approval Timeline:** Vertical stepper dengan 6 nodes:
...
Setiap node menampilkan: role name, approver name (jika sudah proses), timestamp, status icon, notes (jika reject)

**Action Card** (hanya untuk approver yang sedang active):
- "Approve" button (green)
- "Reject" button (Signal Orange)
- Notes textarea (wajib untuk reject)

---

### 11.6 Approval Modal (Reject)
...
**Form:**
- Alasan Penolakan (textarea, required, min 10 karakter)
- Placeholder: "Jelaskan alasan penolakan..."

**Actions:**
- "Batal" (secondary)
- "Tolak Pengajuan" (Signal Orange, destructive)

---

### 11.7 User Profile & 2FA Setup

**Profile Section:** Avatar/initials, Name, Email, Role (badge)

**2FA Settings:**
- Status: Active/Not Configured
- "Enable 2FA" → shows QR code
- QR code untuk Google Authenticator
- Secret key (manual entry)
- Input field untuk verify first code
- "Disable 2FA" button (dengan confirm)
...
| Tablet | 768px - 1024px | Two column untuk detail page, condensed nav |
| Desktop | > 1024px | Full layout, floating nav pill, multi-column |

### Mobile Adaptations:
- Navigation: Collapses to hamburger menu
- Table: Card list dengan stacked info
- Form: Single column, full-width inputs
- Map: Full width, height 300px
- Timeline: Compact vertical stepper
...
- **Keyboard Navigation**: Tab order yang jelas
- **Focus Indicators**: Visible outline pada semua interactive elements
- **ARIA Labels**: Untuk map, file upload, status badges
- **Screen Reader**: Alt text untuk semua images dan icons
- **Error Messages**: Clear dan descriptive, linked ke inputs

---

## 15. Empty States

**Dashboard (No Submissions):**
- Illustration: Empty box/folder
- Title: "Belum Ada Pengajuan"
- Subtitle: "Mulai dengan membuat pengajuan baru"
- Button: "Buat Pengajuan Pertama"

**Approvals (None Pending):**
- Illustration: Checkmark circle
- Title: "Semua Tuntas!"
- Subtitle: "Tidak ada pengajuan yang menunggu approval Anda"
...
**Skeleton Screens:**
- Table rows: Gray pulsing rectangles
- Cards: Pulsing blocks untuk title + content
- Map: Gray placeholder dengan loading spinner

**Button Loading:**
- Spinner icon replacing text atau inline dots
- Disabled state selama loading
...
- Summary di top of form untuk multiple errors

**Network Error:**
- Toast notification (top-right)
- Retry button
- Persistent sampai dismissed atau resolved
...
## 18. Agent Prompt Guide - Application Components

### Example Component Prompts

- "Design a submission detail card: White background, 24px border-radius, 32px padding, soft shadow. Show warehouse name as H3 (20px, weight 500), location with map icon, budget in large typography (32px, weight 500), and embedded map frame (40px radius, 400px height)."
- "Create an approval timeline stepper: Vertical line with 6 circular nodes (24px diameter), connected by 2px line. Active node: Signal Orange fill. Completed nodes: Green fill with checkmark. Pending nodes: Gray outline. Each node has role label to the right."
- "Design a stats card: White background, 24px radius, 32px padding. Show label in uppercase (12px, gray), value in large typography (36px, weight 500, Ink Black), and trend indicator (small, green/red) in corner."
- "Design a file upload card: Dashed border (2px, #D1CDC7), 40px radius, Canvas Cream background. Center: cloud upload icon (48px), title 'Drag & drop files here', subtitle 'or click to browse', supported formats list below."
- "Design a floating navigation pill for dashboard: White background with soft shadow, 999px border-radius, 16px vertical and 40px horizontal padding. Logo left, navigation links center (48px gap), user avatar + logout right."
